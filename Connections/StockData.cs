...
public class StockData {
    ...
    public async Task FetchAndStoreETFDataAsync() {
        var sectorTickerPairs = dataReader.GetSectorETFTickerPairs();
        try {
            using (HttpClient client = new HttpClient()) {
                ...
                foreach (var (sector, ticker) in sectorTickerPairs)
                {
                    ...
                    {
                        dataReader.InsertStockData("SECTOR_ETF_DATA", sector, ticker, stockData);
                    }
                }
